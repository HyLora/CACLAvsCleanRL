...
import wandb
import time
# --- ADDED: We need a deque to store recent rewards ---
from collections import deque

# --- Models (ActorModel, CriticModel) are identical to your other script ---
class ActorModel(nn.Module):
    def __init__(self, lr, input_size, output_size, max_action):
        super(ActorModel, self).__init__()
        self.lr = lr; self.h = 256; self.max_action = max_action
    ...
    def forward(self, x):
        ...
        x = self.linear3(x); x = self.tanh(x); x = x * self.max_action
        return x
...
# --- 2. Main Training Function (with Early Stopping) ---
def main():
    wandb.init(project="cacla-vs-cleanrl-benchmark", name="cacla-early-stopping")
    ...
    # Hyperparameters
    actor_lr = 1e-4; critic_lr = 3e-4; gamma = 0.99
    sigma_start = 1.0; sigma_end = 0.1; sigma_decay = 500
    
    # --- ADDED: Early Stopping Parameters ---
    # Pendulum-v1 is "solved" around -140. Let's aim for -160.
    TARGET_REWARD = -160.0
    # We must be stable for 20 episodes
    REWARD_WINDOW_SIZE = 20
    # This will hold the last 20 rewards
    ...
    print(f"Will stop early if avg reward over {REWARD_WINDOW_SIZE} episodes > {TARGET_REWARD}")
    
    global_step = 0
    episode_num = 0
    
    # --- CHANGED: A 'while True' loop that we 'break' out of ---
    while True:
        ...
        while not terminated and not truncated:
            ...
            with torch.no_grad(): value_new = critic(next_state_tensor)
            value = critic(state_tensor)
            
            if terminated: target = torch.tensor([[reward]], dtype=torch.float32) 
            else: target = reward + gamma * value_new
        ...
        if episode_num % 10 == 0:
            print(f"Ep: {episode_num}, GStep: {global_step}, Reward: {total_reward:.2f}, Avg (last {len(reward_window)}): {avg_reward:.2f}")
        ...
        # Check if we have enough episodes *and* if we hit the target
        if len(reward_window) == REWARD_WINDOW_SIZE and avg_reward >= TARGET_REWARD:
            ...
            print(f"Stopping at episode {episode_num} (global_step {global_step}).")
